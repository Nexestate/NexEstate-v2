...
export function TenantsPage() {
  ...
  const section = searchParams.get('tab') === 'leases' ? 'leases' : 'tenants';
  ...
  const setSection = (id: string) => {
    ...
    if (id === 'leases') next.set('tab', 'leases');
  ...
  return (
    <div className="space-y-6">
      {propertyFilter && (
        <>
          <BackButton to="/broker/tenants" label="חזרה לרשימה המלאה" />
          <PropertySubNav propertyId={propertyFilter} />
        </>
      )}

      <div>
        <h2 className="text-xl font-bold">שוכרים וחוזים</h2>
        <p className="text-sm text-muted-foreground">
          {propertyTitle ? `נכס: ${propertyTitle}` : 'ניהול שוכרים וחוזי שכירות'}
        </p>
        {propertyFilter && (
          <Link to="/broker/tenants" className="mt-1 inline-block text-sm text-primary hover:underline">
            הצג הכל
          </Link>
        )}
      </div>
